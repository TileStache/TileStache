...
def create_tileset(filename, name, type, version, description, format, bounds=None):
    ...
    if format not in ('png', 'jpg',' json', 'pbf'):
        raise Exception('Format must be one of "png" or "jpg" or "json" or "pbf", not "%s"' % format)
...
def get_tile(filename, coord):
    ...
    formats = {'png': 'image/png', 'jpg': 'image/jpeg', 'json':
            'application/json', 'pbf':'application/x-protobuf', None: None}
...
class Provider:
    ...
    def renderTile(self, width, height, srs, coord):
        ...
        formats = {'image/png': 'PNG', 'image/jpeg': 'JPEG',
                'application/json': 'JSON','application/x-protobuf':'PBF', None: None}
        return TileResponse(formats[mime_type], content)

    def getTypeByExtension(self, extension):
        """ Get mime-type and format by file extension.
        
            This only accepts "png" or "jpg" or "json" or "pbf".
        """
        if extension.lower() == 'json':
            ...
            raise KnownUnknown('MBTiles only makes .png and .jpg and .json ',
                    'and pbf tiles, not "%s"' % extension)
