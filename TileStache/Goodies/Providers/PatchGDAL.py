"""
    "provider":
    {
      "class": "TileStache.Goodies.Providers.PatchGDAL:Provider",
        ...
        try:
            ...
            if self.maskband > 0:
                ...
                mask_ds = driver.Create(
                    "/vsimem/alpha", width, height, src_ds.RasterCount, gdal.GDT_Float32
                )
                ...
                [
                    mask_ds.GetRasterBand(i).SetNoDataValue(float("nan"))
                    for i in xrange(1, src_ds.RasterCount + 1)
                ]
            ...
            gdal.ReprojectImage(
                src_ds, area_ds, src_ds.GetProjection(), area_ds.GetProjection(), resample
            )
            if mask_ds is not None:
            ...
            raster = np.moveaxis(
                np.array(
                    [area_ds.GetRasterBand(i).ReadAsArray(0, 0, width, height) for i in channel],
                    dtype=np.uint8,
                ),
                0,
                -1,
            ...
            if mask_ds is None:
                area = Image.fromarray(raster)
            else:
                raise Exception(
                    "The transparency operations aren't ported from the old version -- feel free."
                )
                # a = mask_ds.GetRasterBand(self.maskband).GetMaskBand().ReadRaster(0, 0, width, height)
                # data = b''.join([struct.unpack('ffff', pixel) for pixel in zip(r, g, b, a)])
